...
# --- THE TRUE GHOST TRIGGER ---
# We use an empty placeholder that only Python and JS know about
unlock_trigger = st.empty()
if unlock_trigger.button("", key="hidden_trigger", help=""):
    st.session_state.unlocked = True
    st.rerun()

# CSS to make sure that specific button is invisible and has 0px height
st.markdown("""
    <style>
    ...
    function render() {{
        ...
        if(answer.length === target.length) {{
            if(answer.join('') === target) {{
                ...
                // JS searches for the hidden trigger button in the Streamlit parent
                setTimeout(() => {{
                    const buttons = window.parent.document.querySelectorAll('button');
                    for (const btn of buttons) {{
                        // We target the button by its key/ID generated by Streamlit
